...
export default async (req, res, fastr) => {
  ...
  const [ provider, email, uid ] = Buffer.from(user, 'base64').toString().split("/")
  ...
  const uidOf = contributor => {
    const [, uid] = contributor.avatar.match(/githubusercontent.com\/u\/(\d+).*$/);
    return uid;
  }
  
  const myKarma = contributors => {
    const contributor = contributors.find(each => each.email === email || uidOf(each) === uid );
